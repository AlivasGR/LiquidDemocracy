...
public class mappageServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            ArrayList<String> votenames = VoteDB.getPoll(initid);
            ArrayList<User> userswhovoted = new ArrayList<>();
            out.println("{areas: ");
            out.print("[" + votenames.size());
            for (String s : votenames) {
                User u = UserDB.getUser(s);                
                out.print(", " + u.getCountry()+u.getTown());
            }
            out.println("]");
            out.println("}");           
